...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter { // 1-1. BasicAuthenticationFilter 상속
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		try {
			...
			String username = Jwts.parser().setSigningKey(secretKeyBytes).parseClaimsJws(access_token).getBody().get("username", String.class);
			
			// 1-9. 서명이 정상적으로 되었다면 userId가 null이 아님
			if(username != null) {
